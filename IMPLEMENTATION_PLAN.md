...
- [ ] Create src/server/index.js with native http module (why: zero deps, Express adds 50+ transitive dependencies)
- [ ] Implement core security middleware: localhost binding, path validation with resolve+startsWith (why: prevent directory traversal attacks)
- [ ] Add GET /api/tree endpoint returning JSON structure for commands/, looppool/, agents/, cc\ resources/ (why: IDE needs directory navigation)
- [ ] Add GET /api/file?path=... endpoint with streaming for large files (why: largest file is 2,379 lines)
- [ ] Add PUT /api/file?path=... endpoint with atomic writes using temp file + rename (why: prevent corruption on crash)
- [ ] Set up SSE endpoint /api/events for real-time file change notifications (why: simpler than WebSocket, built into http)
- [ ] Implement fs.watch with debouncing and path filtering (why: prevent notification storms across 426 files)
- [ ] Add npm script "ide": "node src/server/index.js" to package.json (why: standard launch command)
- [ ] Add browser auto-launch to localhost:3456 using platform-specific commands (why: better UX than manual URL)
