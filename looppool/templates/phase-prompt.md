...
> **Note:** Planning methodology is in `agents/lpl-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/looppool/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/lpl:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/looppool/templates/user-setup.md` for full schema and examples
...
See `~/.claude/looppool/workflows/verify-phase.md` for verification logic.
