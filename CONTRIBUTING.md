...
LPL optimizes for **solo developer + Claude workflow**. The release process follows the same principle: complexity lives in automation, not your workflow.
...
Production. Always installable via `npx looppool-cc`.
...
Users opt-in: `npm install looppool-cc@1.10.0-alpha.1`
...
```
Go to: github.com/glittercowboy/looppool/releases/new
Select tag: v1.10.0
Title: v1.10.0
...
- [ ] Follows LPL style (no enterprise patterns, no filler)
...
```bash
# Clone
git clone https://github.com/glittercowboy/looppool.git
cd looppool
