...
<objective>
Create all phases necessary to close gaps identified by `/lpl:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/lpl:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns lpl-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/lpl:audit-milestone` first.
```
...
**Also available:**
- `/lpl:execute-phase {N}` — if plans already exist
...
`/lpl:audit-milestone` — re-audit to verify gaps closed
`/lpl:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/lpl:plan-phase` next
</success_criteria>
